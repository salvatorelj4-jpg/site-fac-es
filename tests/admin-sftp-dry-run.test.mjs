...
test('host-key verification failure is rejected', async () => { await assert.rejects(() => runAdminSftpDryRun({ env:{ OBC_SFTP_DRY_RUN:'true' }, createAgent:() => mockAgent({ hostKeyVerified:false }) }), /SFTP_DRY_RUN_FAILED/); });
